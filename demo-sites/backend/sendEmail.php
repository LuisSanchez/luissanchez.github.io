...
// Awkward but LOG must be flush left
    ...
    /*if (verifyFormToken('form1')) {*/
        ...
        //if (isset($_POST['URL-main'])) {
            ...
            $whitelist = array('req-name','req-email','subject', 'newText');
			...
			$message .= "<tr style='background: #eee;'><td><strong>Nombre del solicitante</strong> </td><td>" . strip_tags($_POST['req-name']) . "</td></tr>";
			$message .= "<tr><td><strong>Correo eléctronico</strong> </td><td>" . strip_tags($_POST['req-email']) . "</td></tr>";
			...
			$message .= "<tr><td><strong>Contenido:</strong> </td><td>" . htmlentities($_POST['newText']) . "</td></tr>";
			$message .= "</table>";
			$message .= "</body></html>";
			...
			$mail->Host       = "ssl://server-0090.dnsprincipal.com"; // SMTP server
			$mail->SMTPAuth   = true;                  // enable SMTP authentication
			$mail->Port       = 465;          // set the SMTP port for the GMAIL server; 465 for ssl and 587 for tls
			$mail->Username   = "no-reply@cooperativadehangares.com"; // Gmail account username
			$mail->Password   = "cooperativa2014";        // Gmail account password

			$mail->SetFrom('no-reply@cooperativadehangares.com', 'no reply'); //set from name

			$mail->Subject = $_POST['subject'];
			//$mail->MsgHTML($body);
			$mail->MsgHTML($message);
			$mail->addCC($_POST['req-email']);
			$mail->AddAddress($_POST['req-email'], $_POST['req-name']);
